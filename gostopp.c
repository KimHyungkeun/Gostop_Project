...
int main()
{
	...
	int winA=0,winB=0,winC=0; //각 플레이어가 마지막으로 이겼는지 졌는지 확인함.
    int saveA=0,saveB=0,saveC=0; //로드한 횟수
	int nagari = 0,nagari_countA = 0,nagari_countB = 0,nagari_countC = 0, nagari_total = 0; //나가리, //나가리 횟수
	int shakeA = 0,shakeB = 0, shakeC = 0, chongtongA = 0,chongtongB = 0,chongtongC = 0; //흔들기와 총통 횟수
	int goA = 0, goB = 0, goC = 0; //고 횟수 세기
	int stopA = 0,stopB = 0,stopC = 0;
	...
	int stop_arr[3] = {stopA,stopB,stopC};
   ...
   if((nagari = Nagari(&A,&_deck,scoreA,scoreB,scoreC)) == -1){//나가리가 나면 다음 판엔 점수가 2배이다 
	...
	CHOICEA: //A차례에서 수행할 명령
		...
		//Display_DeckCard(&_deck);
			...
			scanf("%c",&choice);
	     __fpurge(stdin); //버퍼를 비워주는 역할을 한다
...
Cgo: //C에 대한 잘못된 명령을 다시 수행하기 위해 넣은 레이블
   ...
   if(winA>=1){                                                                            // 그이외의 경우에는 
    scoreA = Bak_Calculate(&B_G,&C_G,scoreA,stopA,goA,goB,goC,shakeA,nagari_total,&kind); //박 계산까지 하여 점수를 낸다
	scoreB = scoreB;
	scoreC = scoreC;
    }
    else if(winB>=1){
    scoreA = scoreA;
    scoreB = Bak_Calculate(&A_G,&C_G,scoreB,stopB,goB,goA,goC,shakeB,nagari_total,&kind); //박 계산까지 하여 점수를 낸다
    scoreC = scoreC;
    }
    else if(winC>=1){
    scoreA = scoreA;
    scoreB = scoreB;
	scoreC = Bak_Calculate(&A_G,&B_G,scoreC,stopC,goC,goA,goB,shakeC,nagari_total,&kind); //박 계산까지 하여 점수를 낸다
   }
}
	...
	moneyC = Money_Calculate_Minus2(scoreA,moneyC);
		...
		else if(goA >=1 && stopB >= 1){ //A가 고를 했는데 B가 stop을 외치면 점수에 관계없이 B가 이김 
		printf("B의 승리입니다.\n");
	...
	moneyC = Money_Calculate_Minus2(scoreB,moneyC);
         ...
         else if(goA >= 1 && stopC >= 1){ //A가 고를 했는데 C가 stop을 외치면 점수에 관계없이 B가 이김 
         	printf("C의 승리입니다.\n");
	...
	moneyB = Money_Calculate_Minus2(scoreC,moneyB);
		...
		if(goB >= 1 && stopA >= 1){ //B가 고를 했는데 A가 stop을 외치면 점수에 관계없이 A가 이김 
		printf("A의 승리입니다.\n");
	moneyA = Money_Calculate_Get(scoreA,moneyA); //점수의 100배만큼 돈을 얻는다 
	moneyB = Money_Calculate_Minus(scoreA,moneyB); //나머지 플레이어들은 A의 점수의 100배만큼 돈을 잃는다.
	moneyC = Money_Calculate_Minus2(scoreA,moneyC);
	...
	moneyC = Money_Calculate_Minus2(scoreB,moneyC);
         ...
         else if(goB >= 1 && stopC >= 1){ //B가 고를 했는데 C가 stop을 외치면 점수에 관계없이 C가 이김 
         	printf("C의 승리입니다.\n");
	moneyC = Money_Calculate_Get(scoreC,moneyC); //점수의 100배만큼 돈을 얻는다
	moneyA = Money_Calculate_Minus(scoreC,moneyA); //나머지 플레이어들은 A의 점수의 100배만큼 돈을 잃는다.
	moneyB = Money_Calculate_Minus2(scoreC,moneyB);
		...
		if(goC >= 1 && stopA >= 1){ //C가 고를 했는데 A가 stop을 외치면 점수에 관계없이 A가 이김 
		printf("A의 승리입니다.\n");
	moneyA = Money_Calculate_Get(scoreA,moneyA); //점수의 100배만큼 돈을 얻는다 
	moneyB = Money_Calculate_Minus(scoreA,moneyB); //나머지 플레이어들은 A의 점수의 100배만큼 돈을 잃는다.
	moneyC = Money_Calculate_Minus2(scoreA,moneyC);
		...
		else if(goC >=1 && stopB >= 1){ //C가 고를 했는데 B가 stop을 외치면 점수에 관계없이 B가 이김 
		printf("B의 승리입니다.\n");
	moneyB = Money_Calculate_Get(scoreB,moneyB); //점수의 100배만큼 돈을 얻는다
	moneyA = Money_Calculate_Minus(scoreB,moneyA); //나머지 플레이어들은 A의 점수의 100배만큼 돈을 잃는다.
	moneyC = Money_Calculate_Minus2(scoreB,moneyC);
	...
	if(choice == 'y'){
		...
		if(scoreA >= scoreB && scoreA >= scoreC && nagari != -1){
			nagari = 0, nagari_total = 0;
			nagari_countA = 0, nagari_countB = 0, nagari_countC = 0; //누적된 나가리 횟수를 초기화함.
			scoreA = 0,scoreB = 0,scoreC = 0;//각 플레이어가 얻은 점수
			goA = 0, goB = 0, goC = 0; //고 횟수 세기
			stopA = 0, stopB = 0, stopC = 0; //스톱 횟수 세기 
			...
			if(goA >= 1 && stopB >=1)
				goto Bgo;
			else if(goA >= 1 && stopC >= 1)
				goto Cgo;
			else
				goto Ago;
		}
		
		else if(scoreB >= scoreA && scoreB >= scoreC && nagari != -1){
			nagari = 0, nagari_total = 0;;
			nagari_countA = 0, nagari_countB = 0, nagari_countC = 0; //누적된 나가리 횟수를 초기화함.
			scoreA = 0,scoreB = 0,scoreC = 0;//각 플레이어가 얻은 점수
			goA = 0, goB = 0, goC = 0; //고 횟수 세기
			...
			if(goB >= 1 && stopA >=1)
				goto Ago;
			else if(goB >= 1 && stopC >= 1)
				goto Cgo;
			else
				goto Bgo;
		}
		
		else if(scoreC >= scoreA && scoreC >= scoreA && nagari != -1){
			nagari=0, nagari_total = 0;;
			nagari_countA = 0, nagari_countB = 0, nagari_countC = 0; //누적된 나가리 횟수를 초기화함.
			scoreA = 0,scoreB = 0,scoreC = 0;//각 플레이어가 얻은 점수
			goA = 0, goB = 0, goC = 0; //고 횟수 세기
...
void Search_And_TakeOpenCard(List* taker,List* p1,List* p2,List* _open, Node* data_pay, Node* data_draw)
  ...
  while(_open -> curr != _open -> tail)
  {
  	if(_open -> curr -> data.month == pay_save){
  		pay_data_count++;
  		_open -> curr = _open -> curr -> next;
  	}
  ...
  while(_open -> curr != _open -> tail)
  {
  	if(_open -> curr -> data.month == draw_save){
  		draw_data_count++;
  		_open -> curr = _open -> curr -> next;
  	}
  ...
  /* 가져가는 방식*/
  if (draw_count == 1 && draw_data_count == 1 && pay_count == 1 && pay_data_count == 2 ) //패 2장 먹기(낸 카드를 중심으로)
	  {
		  _open -> curr = _open -> head -> next;
		   while(_open -> curr != _open -> tail){ //
	  ...
	  else if(draw_count == 1 && draw_data_count == 2 && pay_count == 1 && pay_data_count == 1) //따닥
	  {
		  _open -> curr = _open -> head -> next;
		  while(_open -> curr != _open -> tail){
	 ...
	 taker -> curr = NULL;
	  ...
	  else if(draw_count == 1 && draw_data_count == 3 && pay_count == 1 && pay_data_count == 2 ) //설사 
  	 {   
		 printf("저런 쌌습니다..\n");
  		  
  	 }

  	else if(draw_count == 1 && draw_data_count == 1 && pay_count == 1 && pay_data_count == 3 ) //둘 중 하나 고르기 (낸 카드의 월을 기준으로)
	{
		i=0;
		_open -> curr = _open -> head -> next;
		while(_open -> curr != _open -> tail){
	...
	else if(draw_count == 1 && draw_data_count == 1 && pay_count == 1 && pay_data_count == 4 ) //4장 다 가져가기 (낸 카드의 월을 기준으로)
            {
				_open -> curr = _open -> head -> next;
				while(_open -> curr != _open -> tail){
  	...
  	else if( draw_count == 1  && draw_data_count == 3 && pay_count == 1 && pay_data_count == 1)//둘 중 하나 고르기(드로우한 카드의 월을 기준으로)
           {
			   i=0;
		_open -> curr = _open -> head -> next;
		while(_open -> curr != _open -> tail){
  	...
  	else if(draw_count == 1  && draw_data_count == 2 && pay_count == 1 && pay_data_count == 2) //낸 카드의 월 2장, 드로우한 카드의 월 2장 
      {
		  _open -> curr = _open -> head -> next;
     while(_open -> curr != _open -> tail){
     if( _open -> curr -> data.month == data_pay-> data.month || _open -> curr -> data.month == data_draw -> data.month)
         ...
         if(draw_count == 1 && draw_data_count == 1 && pay_count == 1 && pay_data_count == 4 ) //4장 다 가져가기 (낸 카드의 월을 기준으로)
         	      taker = StealPee(taker,p1,p2); //피 뺏기
         
         if(draw_count == 1 && draw_data_count == 4 && pay_count == 1 &&  pay_data_count == 1 )//4장 다 가져가기(드로우한 카드의 월을 기준으로)
                  taker = StealPee(taker,p1,p2); //피 뺏기

		 if(draw_count == 1 && draw_data_count == 2 && pay_count == 1 && pay_data_count == 1)//따닥
                  taker = StealPee(taker,p1,p2); //피 뺏기
...
int Nagari(List* player,List* _deck,int scoreA,int scoreB,int scoreC) //나가리
{
	...
	if(scoreA >= 3 && scoreB < 3 && scoreC < 3){
		if(player -> head -> next == player -> tail)
	{
      printf("나가리입니다. 다음판을 이기면 점수는 2배~~\n");
	  return -1;
	}
...
{
	int Bee_Gwang_count = 0; //비광 세는 변수 
	...
	{
		if(p_g -> curr -> data.cardname == "11쌍피") 
		(kind -> Pee) += 2; //쌍피이면 2개 있는 것으로 간주하고 

	    if(p_g -> curr -> data.cardname == "12쌍피")
	    (kind -> Pee) += 2; //쌍피이면 2개 있는 것으로 간주하고 
	...
	{
		kind -> Gwang++;
		if(p_g -> curr -> data.cardname == "12비광")
			Bee_Gwang_count = 1;

		if(kind -> Gwang == 3){
			
			if(Bee_Gwang_count >= 1){ //12월의 비광이 있으면 
				printf("비3광 입니다.\n"); //비3광일 경우에는 2점이 추가된다 
				count += 2;
			}
			...
			printf("5광 입니다.\n");
			count += 11; //예전의 광과 합산하여 15점이 되도록 점수를 얻는다
		}
	}
...
int Bak_Calculate(List* p_g2,List* p_g3,int count,int stop,int go,int go2,int go3,int shake,int nagari_count,Score* kind) //박 계산 하기
{
	int i=0;
//광박
...
int Save(Card* Asave,Card* Bsave,Card* Csave,Card* A_Gsave,Card* B_Gsave,Card* C_Gsave,Card* _opensave,Card* _decksave,List* A,List* B,List* C,List* A_G,List* B_G,List* C_G,List* _open,List* _deck,int* nagari,int* shake,int* go,int* score,int* money,int* temp)
{
    ...
    A -> curr = A -> head -> next; //각 curr에 다음 노드를 연결 
	B -> curr = B -> head -> next; //각 curr에 다음 노드를 연결 
	C -> curr = C -> head -> next; //각 curr에 다음 노드를 연결 
	A_G -> curr = A_G -> head -> next; //각 curr에 다음 노드를 연결 
	B_G -> curr = B_G -> head -> next; //각 curr에 다음 노드를 연결 
	C_G -> curr = C_G -> head -> next; //각 curr에 다음 노드를 연결 
	_open -> curr = _open -> head -> next; //각 curr에 다음 노드를 연결 
	_deck -> curr = _deck -> head -> next; //각 curr에 다음 노드를 연결 

	int i = 0;
	while(A -> curr != A -> tail){ //A가 가진 패를 모두 저장한다 
	...
	i=0;
	while(B -> curr != B -> tail){ //B가 가진 패를 모두 저장한다 
	...
	i=0;
	while(C -> curr != C -> tail){ //C가 가진 패를 모두 저장한다 
	...
	i=0;
	while(A_G -> curr != A_G -> tail){ //A_G가 가진 패를 모두 저장한다 
	...
	i=0;
	while(B_G -> curr != B_G -> tail){ //B_G가 가진 패를 모두 저장한다 
	...
	i=0;
	while(C_G -> curr != C_G -> tail){ //C_G가 가진 패를 모두 저장한다 
	...
	i=0;
	while(_open -> curr != _open -> tail){ //_open가 가진 패를 모두 저장한다 
	...
	i=0;
	while(_deck -> curr != _deck -> tail){ //_deck가 가진 패를 모두 저장한다 
...
void Load(Card* Asave,Card* Bsave,Card* Csave,Card* A_Gsave,Card* B_Gsave,Card* C_Gsave,Card* _opensave,Card* _decksave,List* A,List* B,List* C,List* A_G,List* B_G,List* C_G,List* _open,List* _deck,int* nagari,int* shake,int* go,int* score,int* money,int saveA,int saveB,int saveC,int* temp)
{
	...
	Create(B,temp[1]); //temp에 저장된 숫자만큼 각 리스트를 생성한다.
	Create(C,temp[2]); //temp에 저장된 숫자만큼 각 리스트를 생성한다.
	Create(A_G,temp[3]); //temp에 저장된 숫자만큼 각 리스트를 생성한다.
	Create(B_G,temp[4]); //temp에 저장된 숫자만큼 각 리스트를 생성한다.
	Create(C_G,temp[5]); //temp에 저장된 숫자만큼 각 리스트를 생성한다.
	Create(_open,temp[6]); //temp에 저장된 숫자만큼 각 리스트를 생성한다.
	Create(_deck,temp[7]); //temp에 저장된 숫자만큼 각 리스트를 생성한다.
    ...
    int i = 0;
	while(A -> curr != A -> tail){ //A에 대한 리스트들을 로드한다 
	...
	i=0;
	while(B -> curr != B -> tail){ //B에 대한 리스트들을 로드한다 
	...
	i=0;
	while(C -> curr != C -> tail){ //C에 대한 리스트들을 로드한다 
	...
	i=0;
	while(A_G -> curr != A_G -> tail){ //A_G에 대한 리스트들을 로드한다 
	...
	i=0;
	while(B_G -> curr != B_G -> tail){ //B_G에 대한 리스트들을 로드한다 
	...
	i=0;
	while(C_G -> curr != C_G -> tail){ //C_G에 대한 리스트들을 로드한다 
	...
	i=0;
	while(_open -> curr != _open -> tail){ //_open에 대한 리스트들을 로드한다 
	...
	i=0;
	while(_deck -> curr != _deck -> tail){ //_deck에 대한 리스트들을 로드한다
